...
class TwinDeskApp {
    constructor() {
        ...
        this.connectionMode = 'server'; // 'server' | 'client'
        this.connConfig = window.ConnectionConfig ? new window.ConnectionConfig() : null;
    ...
    async showHostInfoThenDashboard(username, port) {
        try {
            // Populate and show host IP info section
            if (this.connectionInfo) {
                if (this.hostPortSpan) this.hostPortSpan.textContent = String(port || 3000);
                if (this.hostIpSpan) {
                    const ip = this.connConfig ? await this.connConfig.getLocalIP() : 'localhost';
                    this.hostIpSpan.textContent = ip || 'localhost';
                }
                this.connectionInfo.style.display = 'block';
            }
        } catch (e) {
            console.warn('Failed to get host IP, defaulting to localhost', e);
            if (this.hostIpSpan) this.hostIpSpan.textContent = 'localhost';
            if (this.connectionInfo) this.connectionInfo.style.display = 'block';
        }

        // Navigate to dashboard immediately but keep connection info visible
        this.navigateToDashboard(username, true);
        
        // Keep connection info visible on dashboard for easier access
        this.showConnectionInfoOnDashboard();
    }
    
    showConnectionInfoOnDashboard() {
        // Create or update connection info display on dashboard
        const dashboardPage = document.getElementById('dashboard-page');
        if (!dashboardPage) return;
        
        let connectionInfoDashboard = document.getElementById('connection-info-dashboard');
        if (!connectionInfoDashboard) {
            connectionInfoDashboard = document.createElement('div');
            connectionInfoDashboard.id = 'connection-info-dashboard';
            connectionInfoDashboard.className = 'connection-info-dashboard';
            
            // Insert after the user corner but before dashboard content
            const dashboardContent = dashboardPage.querySelector('.dashboard-content');
            if (dashboardContent) {
                dashboardContent.insertAdjacentElement('beforebegin', connectionInfoDashboard);
            } else {
                const userCorner = dashboardPage.querySelector('.user-corner');
                if (userCorner) {
                    userCorner.insertAdjacentElement('afterend', connectionInfoDashboard);
                } else {
                    dashboardPage.insertBefore(connectionInfoDashboard, dashboardPage.firstChild);
                }
            }
        }
        
        const hostIp = this.hostIpSpan ? this.hostIpSpan.textContent : 'localhost';
        const hostPort = this.hostPortSpan ? this.hostPortSpan.textContent : '3000';
        
        connectionInfoDashboard.innerHTML = `
            <div class="connection-info-card">
                <div class="card-header">
                    <h3><i class="fa-solid fa-network-wired"></i> Your Host Information</h3>
                    <button id="dashboard-refresh-ip-btn" class="btn btn-small refresh-btn" title="Refresh IP Address">
                ...
                <div class="connection-details">
                    <div class="info-row primary">
                        <div class="info-content">
                            ...
                            <span class="info-value ip-address" id="dashboard-host-ip">${hostIp}</span>
                        </div>
                        <button id="dashboard-copy-ip-btn" class="btn btn-primary copy-btn">
                            <i class="fa-solid fa-copy"></i> Copy IP
                        </button>
                    </div>
                    <div class="info-row secondary">
                        <div class="info-content">
                            <span class="info-label">Port:</span>
                            <span class="info-value port-number">${hostPort}</span>
                        </div>
                        <button id="dashboard-copy-full-btn" class="btn btn-secondary copy-btn">
                ...
                <div class="info-help">
                    ...
                    <span>Share this information with others to let them connect to your session</span>
                </div>
            </div>
        `;
        
        // Add event listeners for dashboard buttons
        const dashboardCopyBtn = document.getElementById('dashboard-copy-ip-btn');
        const dashboardCopyFullBtn = document.getElementById('dashboard-copy-full-btn');
        const dashboardRefreshBtn = document.getElementById('dashboard-refresh-ip-btn');
        
        if (dashboardCopyBtn) {
            dashboardCopyBtn.addEventListener('click', async () => {
                try {
                    await navigator.clipboard.writeText(hostIp);
                    this.showNotification(`IP address ${hostIp} copied to clipboard!`, 'success');
                    // Visual feedback
                    const originalHTML = dashboardCopyBtn.innerHTML;
                    dashboardCopyBtn.innerHTML = '<i class="fa-solid fa-check"></i> Copied!';
                    dashboardCopyBtn.classList.add('success');
                    setTimeout(() => {
                        dashboardCopyBtn.innerHTML = originalHTML;
                        dashboardCopyBtn.classList.remove('success');
                    }, 2000);
                } catch (err) {
        ...
        if (dashboardCopyFullBtn) {
            dashboardCopyFullBtn.addEventListener('click', async () => {
                const fullAddress = `${hostIp}:${hostPort}`;
                try {
                    await navigator.clipboard.writeText(fullAddress);
                    this.showNotification(`Full address ${fullAddress} copied to clipboard!`, 'success');
                    // Visual feedback
                    const originalHTML = dashboardCopyFullBtn.innerHTML;
                    dashboardCopyFullBtn.innerHTML = '<i class="fa-solid fa-check"></i> Copied!';
                    dashboardCopyFullBtn.classList.add('success');
                    setTimeout(() => {
                        dashboardCopyFullBtn.innerHTML = originalHTML;
                        dashboardCopyFullBtn.classList.remove('success');
                    }, 2000);
                } catch (err) {
                    console.error('Copy failed:', err);
                    this.showNotification('Failed to copy full address', 'error');
                }
            });
        }
        ...
        if (dashboardRefreshBtn) {
            dashboardRefreshBtn.addEventListener('click', async () => {
                try {
                    this.showNotification('Refreshing IP address...', 'info');
                    await this.updateHostIPDisplay();
                    // Update the dashboard display with new IP
                    setTimeout(() => {
                        this.showConnectionInfoOnDashboard();
                    }, 500);
                } catch (err) {
                    console.error('Refresh failed:', err);
                    this.showNotification('Failed to refresh IP address', 'error');
                }
            });
        }
    }
    
    async updateHostIPDisplay() {
        console.log('Updating host IP display...');
        try {
            if (this.hostIpSpan) {
                this.hostIpSpan.textContent = 'Getting IP...';
                const ip = this.connConfig ? await this.connConfig.getLocalIP() : 'localhost';
                console.log('Retrieved IP:', ip);
                this.hostIpSpan.textContent = ip || 'localhost';
                
                // Also update port if available
                if (this.hostPortSpan) {
                    try {
                        const port = await ipcRenderer.invoke('get-server-port');
                        this.hostPortSpan.textContent = String(port || 3000);
                        console.log('Retrieved port:', port);
                    } catch (e) {
                        console.log('Could not get port via IPC, using default');
                        this.hostPortSpan.textContent = '3000';
                    }
                }
            }
        } catch (error) {
            console.error('Failed to update host IP display:', error);
            if (this.hostIpSpan) {
                this.hostIpSpan.textContent = 'localhost';
            }
        }
    }
    ...
    initialize() {
        ...
        // Initialize IP display if in server mode
        setTimeout(() => {
            if (this.connectionMode === 'server' && this.connectionInfo) {
                this.updateHostIPDisplay();
            }
        }, 1000);
        
        // Add global test function for debugging
        window.testIPFunction = () => {
            console.log('Test IP function called');
            this.updateHostIPDisplay();
        };
        
        // Add global connect test function
        window.testConnect = () => {
            console.log('=== GLOBAL TEST CONNECT CALLED ===');
            try {
                const usernameInput = document.getElementById('username-input');
                const username = usernameInput?.value?.trim() || 'TestUser';
                console.log('Username from global function:', username);
                ...
                if (usernamePage && dashboardPage) {
                    console.log('Hiding username page...');
                ...
                if (this && this.navigateToDashboard) {
                    console.log('Also calling class method...');
                    this.navigateToDashboard(username, false);
                }
            } catch (error) {
                console.error('Error in testConnect:', error);
            }
        };
    }
    ...
    initializeElements() {
        ...
        // Username page elements - with retry logic
        setTimeout(() => {
            ...
            console.log('Connect button exists:', !!this.connectBtn);
            console.log('Connect button type:', typeof this.connectBtn);
            ...
            if (this.connectBtn) {
                ...
                this.connectBtn.addEventListener('click', (e) => {
                    ...
                    e.preventDefault();
                    e.stopPropagation();
                    const username = this.usernameInput?.value?.trim() || 'TestUser';
                    console.log('Navigating with username:', username);
                    this.navigateToDashboard(username, false);
                });
                console.log('Event listener added successfully');
            } else {
                console.error('Connect button still not found after delay!');
            }
        }, 1000);
        ...
        this.modeRadios = document.querySelectorAll('input[name="connection-mode"]');
        this.serverIpContainer = document.getElementById('server-ip-container');
        this.serverIpInput = document.getElementById('server-ip-input');
        this.connectionInfo = document.getElementById('connection-info');
        this.hostIpSpan = document.getElementById('host-ip');
        this.hostPortSpan = document.getElementById('host-port');
    ...
    bindEvents() {
        ...
        if (this.connectBtn) {
            ...
            this.connectBtn.addEventListener('click', (e) => {
                ...
                e.stopPropagation();
                console.log('Connect button clicked!');
                console.log('Username input value:', this.usernameInput?.value);
                
                // Simple direct navigation test
                const username = this.usernameInput?.value?.trim() || 'TestUser';
                console.log('Attempting direct navigation with username:', username);
                this.navigateToDashboard(username, false);
            });
        } else {
        ...
        // Mode radio change
        if (this.modeRadios && this.modeRadios.length) {
            this.modeRadios.forEach(r => {
                r.addEventListener('change', () => {
                    this.connectionMode = r.value; // 'server' or 'client'
                    // Toggle IP input visibility
                    if (this.serverIpContainer) {
                        this.serverIpContainer.style.display = (r.value === 'client') ? 'block' : 'none';
                    }
                    // Show/hide connection info based on mode
                    if (this.connectionInfo) {
                        this.connectionInfo.style.display = (r.value === 'server') ? 'block' : 'none';
                        // If server mode, immediately try to get IP
                        if (r.value === 'server') {
                            this.updateHostIPDisplay();
                        }
                    }
        ...
        // Copy IP button
        if (this.copyIpBtn) {
            this.copyIpBtn.addEventListener('click', async () => {
                const text = this.hostIpSpan ? this.hostIpSpan.textContent : '';
                try {
                    await navigator.clipboard.writeText(text || '');
                    this.showNotification(`IP address ${text} copied to clipboard!`, 'success');
                    // Change button text temporarily to show success
                    const originalText = this.copyIpBtn.textContent;
                    this.copyIpBtn.textContent = 'Copied!';
                    this.copyIpBtn.style.backgroundColor = '#28a745';
                    setTimeout(() => {
                        this.copyIpBtn.textContent = originalText;
                        this.copyIpBtn.style.backgroundColor = '';
                    }, 2000);
                } catch (err) {
                    console.error('Copy failed:', err);
                    this.showNotification('Failed to copy IP address', 'error');
                    ...
                    if (this.hostIpSpan) {
                        ...
                        range.selectNodeContents(this.hostIpSpan);
                        const selection = window.getSelection();
                        selection.removeAllRanges();
                        selection.addRange(range);
                        this.showNotification('IP address selected - press Ctrl+C to copy', 'info');
                    }
                }
            });
        }
    ...
    showPage(pageId) {
        ...
        console.log('Available pages:', {
            ...
            usernamePage: !!this.usernamePage,
        ...
        switch (pageId) {
            case 'welcome':
                ...
                break;
            case 'username':
                if (this.usernamePage) {
                    this.usernamePage.style.display = 'flex';
                    console.log('Username page shown');
                }
                break;
            case 'dashboard':
    ...
    async connectToServer() {
        ...
        console.log('usernameInput element:', this.usernameInput);
        const username = this.usernameInput?.value?.trim() || '';
        console.log('Username value:', username);
        console.log('Username length:', username.length);
        
        if (!username) {
            console.log('No username provided, showing error');
            this.showNotification('Please enter a username', 'error');
            return;
        }
        ...
        // For immediate testing - navigate to dashboard right away
        ...
        if (this.socket && this.socket.connected) {
            ...
            this.navigateToDashboard(username);
            return;
        }
        ...
        // Set up a timeout to navigate to dashboard even if connection fails
        const connectionTimeout = setTimeout(() => {
            console.log('Connection timeout - proceeding to dashboard in offline mode');
            this.navigateToDashboard(username, false);
        ...
        try {
            ...
            if (this.connectionMode === 'server') {
                // Server mode: get local electron server port via IPC
                if (!this.serverPort) {
                    ...
                    try {
                        ...
                        console.log('IPC call failed, proceeding in offline mode:', ipcError);
                ...
                if (!this.serverPort) {
                    console.log('No port received from server, proceeding in offline mode');
                    clearTimeout(connectionTimeout);
                    this.navigateToDashboard(username, false);
                    return;
                }
                ...
                // Client mode: use provided server IP and default port 3000
                const ip = (this.serverIpInput?.value || '').trim();
                if (!ip) {
                    this.showNotification('Please enter server IP to join session', 'error');
                    clearTimeout(connectionTimeout);
                    return;
                }
                targetHost = ip;
            ...
            console.log(`Attempting to connect to http://${targetHost}:${targetPort}`);
            this.socket = io(`http://${targetHost}:${targetPort}`, {
                timeout: 3000,
                forceNew: true,
            ...
            this.socket.on('connect', () => {
                ...
                if (this.connectionMode === 'server') {
                    this.showHostInfoThenDashboard(username, targetPort);
                } else {
                    this.navigateToDashboard(username, true);
                }
            });
    ...
    navigateToDashboard(username, isOnline = false) {
        console.log(`Navigating to dashboard - Username: ${username}, Online: ${isOnline}`);
        
        this.currentUser = { username, id: this.socket?.id || 'offline-' + Date.now() };
        ...
        if (isOnline) {
            ...
            // Show a placeholder message for offline mode
    ...
    showOfflineMode() {
        ...
        if (noUsersPlaceholder) {
            ...
            const placeholderText = noUsersPlaceholder.querySelector('p');
            if (placeholderText) {
                placeholderText.textContent = 'Working offline - Server connection unavailable';
            }
        }
    }
