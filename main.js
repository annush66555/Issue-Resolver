...
// Check if this is the first instance or additional instance
const gotTheLock = app.requestSingleInstanceLock();

if (!gotTheLock) {
  // This is a second instance - just create window and connect to existing server
  console.log('Second instance detected - connecting to existing server');
  
  app.whenReady().then(() => {
    createWindow();
    
    // Set up IPC handler to return the default port for client connections
    ipcMain.handle('get-server-port', () => {
      console.log('IPC: get-server-port called (client mode), returning: 3000');
      return 3000; // Connect to the first instance's server
    });
  });
} else {
  // This is the first instance - start server and create window
  ...
  app.whenReady().then(async () => {
    try {
      console.log('First instance - starting server and window...');
      ...
      ipcMain.handle('get-server-port', () => {
        console.log('IPC: get-server-port called (server mode), returning:', serverPort);
        return serverPort;
      });
